...
async function login (body) {
  try {
    ...
    if (mentor.status === 'REJECTED' || mentor.status === 'PENDING') {
      throw new CustomError(`Your profile approval status is ${mentor.status.toLowerCase()}`)
    }
...
async function forgotPassword (body) {
  try {
    ...
    const mailSent = await sendMail('hello@simpliclarify.com', mentor.email, subject, mailBody)
...
async function updatePassword (body) {
  try {
    ...
    if (foundUser) {
      const matchPwd = await Methods.comparePasswords(oldPassword, foundUser.password)
      if (matchPwd) {
        foundUser.password = newPassword
        const updatedUser = await foundUser.save()
        return
      } else {
        throw new CustomError('Looks like you have entered wrong password, please try again!')
      }
    } else {
