...
async function capturePayout (body) {
  try {
    const { RAZORPAYX: { RAZORPAYX_KEYID, RAZORPAYX_KEYSECRET, RAZORPAYX_ACCOUNT_NUMBER }, ALLOW_PAYOUTS } = RAZORPAYX_CONFIG

    const { mentorId, bookingIds, account_type, bank_account } = body

    // ALLOW_PAYOUTS set to 0 -> Block payouts
    if (!parseInt(ALLOW_PAYOUTS)) {
      throw new CustomError({ statusCode: 400, message: 'Sorry! Payouts are temporarily disabled' })
    }
    ...
    const contactObject = {
      name, email, contact: contactNumber, reference_id: mentorId
    }
    ...
    if (Object.keys(error).length > 0) {
      if (error.description === 'Your account does not have enough balance to carry out the payout operation.') {
...
async function fetchPayouts () {
  try {
    const { RAZORPAYX: { RAZORPAYX_KEYID, RAZORPAYX_KEYSECRET, RAZORPAYX_ACCOUNT_NUMBER } } = RAZORPAYX_CONFIG
